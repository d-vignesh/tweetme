...
class TweetTestCase(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(username="test1", password="testpassword")
        Tweet.objects.create(content="test tweet1", user=self.user)
        Tweet.objects.create(content="test tweet2", user=self.user)
        Tweet.objects.create(content="test tweet3", user=self.user1)
    ...
    def test_tweet_create_api_view(self):
        ...
        client = self.get_client()
        response = client.post(
            "/api/tweets/create/", request_data
        )
        self.assertEqual(response.status_code, 201)
